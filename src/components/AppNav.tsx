...
import { Home, MapPin, LayoutGrid, Video, Github, Sparkles, Smartphone, ChevronLeft, ChevronRight } from "lucide-react";

interface AppNavProps { collapsed: boolean; onToggle: () => void; }

const themeItems = [
  { href: "/", label: "Home", icon: Home },
  { href: "/morocco", label: "Morocco", icon: MapPin },
];
const genericItems = [
  ...
  return (
    <aside className="fixed left-0 top-0 h-full bg-zinc-950/95 border-r border-zinc-800 flex flex-col transition-[width] duration-200 z-40" style={{ width: collapsed ? 52 : 180 }}>
      <div className="flex items-center justify-between p-3 border-b border-zinc-800">
        {!collapsed && <span className="text-sm font-semibold text-zinc-100 truncate">MTD</span>}
